...
jQuery(document).ready(function(){ 
	...
	if(temp_name != ""){
	    ...
	    if(user_designed_template != true) {
	          ...
	          for(var i=0;i<params.length;i++) {
	              if(params[i]["type"] == "text") {
	                  ...
	                  jQuery("#"+id).inlineEdit(params[i]["type"]);
						...
						//jQuery(".pm_display_fields option:'+"fields_required_select"+'").attr('selected', 'selected')	
	               ...
	               jQuery("#pm_exclamation_icon").attr('src',pm_plugin_url+"/images/tick-icon.png").css("opacity","1");
					  ...
					  if(button_type=='1'){
					  	...
					  	jQuery("#pm_cta_button").css('width','100%');
					  	jQuery("#pm_cta_button").find('.pmfb_btn_ico').hide();
					  	jQuery("#pm_cta_button").find('.pmfb_btn_txt_sub').hide();
					  }else{
					  	button_fluid=button_type;
					  	jQuery("#pm_cta_button").css('width','auto');
					  }
	                  jQuery("#pm_cta_button").attr('href', url);
	                  jQuery("#pm_cta_button").data('leadbox', lead_id);

	                  // if(jQuery.trim(txt) == "") {
	                  // 	txt = "CTA Button";
	                  // }
	                  //jQuery("#pm_cta_button").text(txt);	
	                  jQuery("#pm_cta_button").find("span.pmfb_btn_ico").first().children('i').attr('class',left_icon);
	                  jQuery("#pm_cta_button").find("span.pmfb_btn_ico").last().children('i').attr('class',right_icon);
	                  jQuery("#pm_cta_button").find('.pmfb_btn_txt_main').text(txt);
	                  jQuery("#pm_cta_button").find('.pmfb_btn_txt_sub').text(sub_txt);
	                  //alert(btn_class);
	     ...
	     //              if(btn_class == "") {
	                  ...
	                  var id = params[i]["id"];	   		
	                  jQuery("#"+id).inlineEdit(params[i]["type"]);
		            	jQuery("#"+id+' #pm_cta_button').click(function(e){
						    e.preventDefault();
						});
		...
		if(template == null || template == "" ){
			...
			if(!pm_temp_arr) {
				jQuery('#ajax_load_temp').load('<?php echo admin_url("admin-ajax.php?action=plug_load_template&data=") ?>'+template,function(){
					...
					//jQuery("#pm_cta_button").text("CTA Subscribe");
					...
					jQuery("#pm_cta_button").removeClass();				
		 		    ...
		 		    jQuery("#pm_cta_button").addClass("pm_default_btn pmfb_btn");
					jQuery(".pmedit").each(function(){				
				        ...
				        if(edit_type == "text") {
				        	...
				        	jQuery("#"+this.id).inlineEdit(edit_type);
				   			...
				   			jQuery("#"+this.id).inlineEdit(edit_type);
				   		  	jQuery("#"+this.id+' #pm_cta_button').click(function(e){
							    e.preventDefault();
							});
				   		}				
					});   
	...
	jQuery("#save_btn").click(function() { 
		...
		//if(!jQuery("#pm_cta_btn").prop("checked", true)) {
			...
			jQuery(".pmedit").each(function(){
		        ...
		        if(edit_type == "text") {
	                ...
	                var name_input =  jQuery("#pm_name_field").val();
		   			...
		   			var id 		   	 = jQuery(this).attr('id');
		   			var txt 	   	 = jQuery("#pm_cta_button").find(".pmfb_btn_txt_main").text();
		   			var sub_text   	 = jQuery("#pm_cta_button").find(".pmfb_btn_txt_sub").text();
		   			var left_icon  	 = jQuery("#pm_cta_button").find("span.pmfb_btn_ico").first().children('i').attr('class');
		   			var right_icon 	 = jQuery("#pm_cta_button").find("span.pmfb_btn_ico").last().children('i').attr('class');
		   			var btn_class  	 = jQuery("#pm_cta_button").attr("class") ;	
	                var url 		 = jQuery("#pm_cta_button").attr('href');
	                var lead_id      = jQuery("#pm_cta_button").data('leadbox');
	                var button_type  = button_fluid;
	                var button_style = JSON.stringify(jQuery("#pmfb_custom_btn_style").html());
	                json_params.push({"type":edit_type,"id":id,"params":{"right_icon":right_icon,"left_icon":left_icon,"sub_text":sub_text,"text":txt,"btn_class":btn_class, "url":url, "lead_id":lead_id,"button_fluid":button_type,"button_style":button_style }});
			   	}			
			}); 
		}
		return json_params;
