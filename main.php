...
function support_submenu_page_callback(){
	set_js_globals();
...
function pm_admin_scripts($hook) {
	...
	if('admin_page_edit_template_submenu-page' == $hook ) {
		...
		wp_register_script('pm_codemirror_js',plugins_url('js/pm_codemirror.js', __FILE__), array('jquery'));
	    wp_enqueue_script('pm_codemirror_js');
	    wp_register_script('pm_codemirror_css', plugins_url('js/pm_codemirror_css.js', __FILE__));
